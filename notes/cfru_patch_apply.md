...
## Path A — TARGETED attack + ER@K (the §8.5 axis switch)

**Why.** The untargeted `fedAttack` did not degrade utility at a realistic malicious fraction
(smoke: HR_poison ≈ HR_clean, all 4 models within ~0.006), so the utility/UDR axis has no effect to
decompose. CFRU's released code ships **no** targeted attack and **no** ER@K. So we added both.

**Edits (applied to `external/CFRU`):**
- `utils/fflow.py`: add `targetPromo` to `attack_methods`; add `--target_item` (default −1 =
  auto-pick the least-popular item, deterministic, so clean exposure ≈ 0 and the same item is used
  across clean/poison/retain). Prints `target_item = ...` at startup.
- `benchmark/toolkits.py`: `NCFData.ng_sample_target(target_item, malicious_users)` — normal pos/neg
  for everyone PLUS inject `(malicious_user, target_item)` positives (`num_ng` copies/user) so the
  shared target embedding is pushed up. `ClassifyCalculator.test_target_exposure(...)` — ER@K =
  fraction of test users with the target item in top-K of their candidate pool ∪ {target}.
- `algorithm/CFRU.py`: `Client.train` — `targetPromo` branch calls `ng_sample_target` each epoch
  (returns `process_grad`, so the attacker contribution is what CFRU later unlearns). `Server`
  gains `test_target_exposure(model)` (global, like `Server.test`). `save_result` stores
  `er`/`er_unlearn` (unlearn run) and `er_retain` (retrain run) plus `target_item`.
- `scripts/run_unlearning_gap.py`: `ATTACK="targetPromo"`, `--target_item`, ER columns
  (`er_clean/er_poison/er_retain/er_unlearned`), `attack_saturated = ER_poison ≥ 0.30`.

**Validated statically (CPU, tiny tensors):** injection puts target as a positive only for malicious
users (correct counts); ER@K returns [0,1], is ~0 for a cold target in a 100-item pool and 1.0 for a
boosted target. Full behavior (attack raises ER over 40 rounds; CFRU unlearn lowers it) = the H100 smoke.

**Paper framing:** transparent minimal target-item promotion (PipAttack/FedRecAttack family) used as a
**structural probe** of carrier-vs-forget-set, not a strongest-attack claim. Metric = ER@K / ARR@K
(aligns with §7/§8), NOT UDR.
