...
import importlib
# import torchvision.transforms as transforms  # CFRU patch: unused, avoids torchvision dep
...
class ClassifyCalculator(BasicTaskCalculator):
	...
	def test(self, model, test_loader, top_k, users_test=None):
		...
		if users_test == None:
			for user, item_i, item_j in test_loader:
				...
				item_j = item_j.to(self.device) # not useful when testing
	...
	def get_loss_variance(self, model, data, users_set, var_config, epoch_cur, score_cand_all, score_pos_all, Mu_idx, candidate_cur, train_iddict, option=None):
		...
		# numerical guard: constant-score rows (max==min) -> 0/0 NaN -> np.random.choice crash
