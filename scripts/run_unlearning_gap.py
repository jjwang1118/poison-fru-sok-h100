...
Attack = targetPromo (target-item promotion). Primary metric = ER@K (exposure of
the target item). HR/NDCG (utility) are also recorded so utility-vs-exposure can be
contrasted (utility ~ unchanged while exposure differs is the headline).
...
ANCHOR        = "CFRU-targetPromo-ML1M-NCF"
...
ATTACK        = _envstr("ATTACK", "targetPromo")     # "fedAttack" = old untargeted/utility axis
